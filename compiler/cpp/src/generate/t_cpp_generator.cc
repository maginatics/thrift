...
void t_cpp_generator::generate_struct_definition(ofstream& out,
  ...
  // Create a setter method for each field
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    if (pointers) {
      continue;
    }
    ...
    out << endl << indent() <<
    "void set" << get_cap_name((*m_iter)->get_name()) <<
    "(" << type_name((*m_iter)->get_type(), false, true);

    out << " val) {" << endl << indent() << indent() <<
      (*m_iter)->get_name() << " = val;" << endl;
    ...
    if (is_optional) {
      out << indent() << indent() <<
        "__isset." << (*m_iter)->get_name() << " = true;" << endl;
    }
    out << indent()<< "}" << endl;
  }
